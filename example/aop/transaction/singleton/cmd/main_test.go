...
func (a *App) TestRun(t *testing.T) {
	err := a.TradeService.DoTradeWithTxAddMoneyFailed(1, 2, 100)
	assert.NotNil(t, err)
	...
	err = a.TradeService.DoTradeWithTxFinallyFailed(1, 2, 100)
	assert.NotNil(t, err)
	assert.Equal(t, "finally failed", err.Error())
	assert.Equal(t, 100, a.BankService.GetMoney(1))
	assert.Equal(t, 100, a.BankService.GetMoney(2))

	err = a.TradeService.DoTradeWithTxSuccess(1, 2, 100)
	assert.Nil(t, err)
