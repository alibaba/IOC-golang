...
func Test_parseConfigSource(t *testing.T) {
	...
	t.Run("test with env source ", func(t *testing.T) {
		...
		assert.Nil(t, LoadConfigByPrefix("autowire.normal.Redis.Impl.param", redisConfig))
		assert.Equal(t, "0", redisConfig.DB)
		assert.Equal(t, "localhost:6379", redisConfig.Address)

		assert.Nil(t, LoadConfigByPrefix("autowire.normal.Redis.Impl.env-redis.param", redisConfig))
		assert.Equal(t, "1", redisConfig.DB)
		assert.Equal(t, "localhost:16379", redisConfig.Address)

		assert.Nil(t, LoadConfigByPrefix("autowire.normal.Redis.Impl.normal-redis.param", redisConfig))
